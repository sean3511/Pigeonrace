...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('image/piegon_album_autumn/data.json')  // 請將路徑替換為實際 JSON 文件的路徑
...
function createTileElement(parsedData) {
    ...
    // 使用縮略圖作為背景圖像
    tile.style.backgroundImage = `url('image/piegon_album_autumn/${parsedData.title}/${parsedData.thumbnail}')`;
    ...
    tile.addEventListener('click', function() {
        ...
        images.forEach(imageName => {
            ...
            img.src = `image/piegon_album_autumn/${parsedData.title}/${imageName}`;
